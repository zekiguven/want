...
type
  ...
  protected
    ...
    FDCPPath : TPath;
    FSource  : TPath;
    FHppOutputPath : TPath;
    FObjOutputPath : TPath;
    FBpiOutputPath : TPath;
    FGenerateCFiles: string;
  ...
  published
    ...
    property hppoutput :TPath  read FHppOutputPath write FHppOutputPath;
    property objoutput :TPath  read FObjOutputPath write FObjOutputPath;
    property bpioutput :TPath  read FBpiOutputPath write FBpiOutputPath;
    property generate  :string read FGenerateCFiles write FGenerateCFiles;
...
begin
 ...
 if (Pos(':', Line) <> 0) then
   match:=regex.Match('^(.*)(\([0-9]+\)) *([HWEF][a-z]+:.*)$', Line);

 if match.Success then
 begin

   with match do
     Line := ToRelativePath(ToPath(Groups.Item[1].Value)) + ' ' + Groups.Item[2].Value + #10 + Groups.Item[3].Value;
...
begin
  ...
  begin
    ...
    if hppoutput <> '' then
    begin
        Log(vlVerbose, 'hppoutput=' + ToRelativePath(hppoutput));
        Result := Result + PathOpt('NH', hppoutput);
    end
    else
     Result := Result + PathOpt('NH', dcuoutput);

    if objoutput <> '' then
    begin
      Log(vlVerbose, 'objoutput=' + ToRelativePath(objoutput));
      Result := Result + PathOpt('NO', objoutput);
    end
    else
      Result := Result + PathOpt('NO', dcuoutput);

    if bpioutput <> '' then
    begin
      Log(vlVerbose, 'bpioutput=' + ToRelativePath(bpioutput));
      Result := Result + PathOpt('NB', bpioutput);
    end
    else
      Result := Result + PathOpt('NB', dcuoutput);


    Log(vlVerbose, 'generate=' + generate);
    Result := Result + Format(' -%s', [generate] );
