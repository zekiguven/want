...
begin
  ...
  AboutToScratchPath(zipfile);
  //FZipStream := Stream.Create(zipfile);
  try
...
begin
  ...
  FUnzipStream := TZipFile.Create;
  //FUnzipStream.FileNames
  // " ExtractZipFile(zipfile);
  try

    with FUnzipStream do
    begin
      if Length(FUnzipStream.FileNames) > 0 then
      begin
        Log('Unzipping %d files to "%s"', [Length(FileNames), ToPath]);

        for e := 0 to Length(FileNames)-1 do
        begin
          Log(vlVerbose, FileNames[e]);
          Entry := MovePath(FileNames[e], ToPath);
          AboutToScratchPath(Entry);
          ExtractZipFile(FileNames[e], ToPath);
        end;
      end;
    end;
  finally
