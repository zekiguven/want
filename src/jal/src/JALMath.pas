...
begin
  ...
  if S <> '' then begin
     ...
     while (i <= Length(s)) and (s[i] in ['0'..'9', FormatSettings.ThousandSeparator]) do begin
         if (S[i] = FormatSettings.ThousandSeparator) then begin
            if DecimalSeen then
               Exit;
            ThousandPos := i
         end;
     ...
     if (i <= Length(S)) and (S[i] = FormatSettings.DecimalSeparator) then begin
         Inc(i);
         if DecimalSeen then
